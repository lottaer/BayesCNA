...
your data. The provided data should be a numeric vector.
...
purity, i.e. is noisy. The default value for BCP is $p_0 = 0.2$, which
in general is too large for copy number data from liquid biopsies. Using
$p_0 = 0.05$ can be favorable in poor quality cases, and for cases with
higher coverage/purity, we suggest using $p_0 = 0.01$. We choose 500
burnin iterations and 2000 Markov Chain Monte Carlo (MCMC) samples. The
burnin samples are not used for estimating the posterior changepoint
probabilities.
